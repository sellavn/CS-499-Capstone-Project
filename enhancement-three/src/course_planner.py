...
class CoursePlanner:
    ...
    def __init__(self, use_database: bool = False, db_path: str = 'course_catalog.db'):
        ...
        self.db = DatabaseManager(db_path) if use_database else None
    ...
    def save_to_database(self) -> bool:
        """
        save current courses to database
        
        migrates in-memory data to the database
        ...
        try:
            ...
            for course in self.courses:
                course_id = self.db.add_course(course.course_number, course.name)
                if course_id:
                    course_id_map[course.course_number] = course_id
            
            # Insert prerequisites
            for course in self.courses:
                for prereq in course.prerequisites:
                    self.db.add_prerequisite(course.course_number, prereq)
    ...
    def get_course_count(self) -> int:
        ...
        if self.use_database and self.db:
            return self.db.get_course_count()
